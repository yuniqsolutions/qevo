...
export interface CookiesAPI {
	 ...
	 * @since Chrome 132
	 */
	getPartitionKey(details: FrameDetails): Promise<{
		partitionKey: CookiePartitionKey;
	}>;
	/**
...
declare class QevoCookies extends QevoLogger {
	 ...
	 * The partition key for the frame indicated
	 *
	 * @param details - Details to identify the frame (documentId / frameId /
	 * tabId)
	 * @returns The partition key for the indicated frame
	 * @throws If the API is unavailable (e.g. Firefox, or Chrome < 132)
	 * @since Chrome 132
	 *
	 * @example
	 * ```typescript
	 * const { partitionKey } = await cookies.getPartitionKey({ tabId: 42 });
	 * ```
	 */
	getPartitionKey(details: FrameDetails): Promise<{
		partitionKey: CookiePartitionKey;
	 ...
	 *
	 * Matches the native `chrome.cookies.onChanged` /
	 * `browser.cookies.onChanged` event shape.
	 *
	 * @example
	 * ```typescript
	 * cookies.onChanged.addListener((changeInfo) => {
	 *   console.log(changeInfo.cookie.name, changeInfo.removed, changeInfo.cause);
	 * });
	 * ```
	 */
	get onChanged(): CookiesAPI["onChanged"];
	/**
...
/**
 * Generic native runtime event object.
 *
 * Mirrors the browser's `chrome.events.Event` / `browser` event surface
 * (`addListener` / `removeListener` / `hasListener` / `hasListeners`) while
 * staying fully typed over its listener signature, so events such as
 * `onConnectExternal` or `onRestartRequired` keep precise types without
 * resorting to `any`.
 *
 * @typeParam TListener - The exact listener signature for this event.
 */
export interface RuntimeEvent<TListener extends (...args: never[]) => unknown> {
	/**
	 * Registers an event listener.
	 *
	 * @param callback - The listener invoked when the event fires.
	 */
	...
	/**
	 * Checks whether a given listener is registered.
	 *
	 * @param callback - The listener function to look up.
	 */
	hasListener(callback: TListener): boolean;
	/**
	 * Checks whether any listeners are registered for this event.
	 */
	hasListeners(): boolean;
}
/**
 * Listener for `runtime.onRestartRequired` (ChromeOS kiosk).
 *
 * @param reason - Why the restart is required (`app_update`, `os_update`, or
 * `periodic`).
 */
export type RestartRequiredListener = (reason: `${chrome.runtime.OnRestartRequiredReason}`) => void;
/**
 * An object containing information about the current browser.
 *
 * Returned by {@link QevoRuntime.getBrowserInfo} (Firefox only).
 */
...
/**
 * Details of a `runtime.onPerformanceWarning` event (Firefox only).
 */
export interface PerformanceWarningDetails {
	...
	/** The performance warning event severity. */
	severity: "low" | "medium" | "high";
	/** The `tabs.Tab` that the performance warning relates to, if any. */
	tabId?: number;
	/** An explanation of what the warning means, and how to address it. */
	description: string;
}
/**
 * Listener for `runtime.onPerformanceWarning` (Firefox only).
 *
 * @param details - The performance warning details.
 */
export type PerformanceWarningListener = (details: PerformanceWarningDetails) => void;
/**
...
declare class QevoRuntime extends QevoLogger {
	 ...
	 *
	 * @param filter - A filter to find matching contexts. A context matches if
	 * it matches all specified fields in the filter. Any unspecified field in
	 * the filter matches all contexts.
	 * @returns Matching contexts, or an empty array when the API is unavailable
	 * (Chrome < 116 / non-MV3 / Firefox without support).
	 * @since Chrome 116 MV3.
	 *
	 * @example
	 * ```typescript
	 * const [worker] = await runtime.getContexts({ contextTypes: ['BACKGROUND'] });
	 * const allTabs = await runtime.getContexts({ contextTypes: ['TAB'] });
	 * ```
	 */
	 ...
	 *
	 * @typeParam R - Expected response shape.
	 * @param application - The name of the native messaging host.
	 * @param message - The message that will be passed to the native messaging
	 * host.
	 * @returns The host's response.
	 * @throws If the host is unavailable or messaging fails.
	 *
	 * @example
	 * ```typescript
	 * const pong = await runtime.sendNativeMessage<{ ok: boolean }>(
	 *   'com.example.host',
	 *   { type: 'ping' }
	 * );
	 * ```
	 */
	sendNativeMessage<R = unknown>(application: string, message: object): Promise<R>;
	/**
	 ...
	 *
	 * @returns The background page window, or `undefined` if there is none.
	 * @deprecated since Chrome 133. Background pages do not exist in MV3
	 * extensions.
	 */
	getBackgroundPage(): Promise<Window | undefined>;
	/**
	 ...
	 * 122.
	 *
	 * @returns The package directory entry, or `undefined` if unavailable.
	 */
	getPackageDirectoryEntry(): Promise<FileSystemDirectoryEntry | undefined>;
	/**
	 ...
	 * delayed. If called with a value of `-1`, the reboot will be cancelled.
	 * It's a no-op in non-kiosk mode. It's only allowed to be called repeatedly
	 * by the first extension to invoke this API.
	 *
	 * Can return its result via Promise in Manifest V3 or later since Chrome 99.
	 *
	 * @param seconds - Delay before reboot, or `-1` to cancel a pending reboot.
	 * @since Chrome 53
	 *
	 * @example
	 * ```typescript
	 * await runtime.restartAfterDelay(60); // reboot in 1 minute
	 * await runtime.restartAfterDelay(-1); // cancel
	 * ```
	 */
	...
	/**
	 * Cookies API surfaced on the runtime.
	 *
	 * Returns the full {@link QevoCookies} API, which covers the entire native
	 * `chrome.cookies` / `browser.cookies` surface (`get`, `getAll`, `set`,
	 * `remove`, `getAllCookieStores`, `getPartitionKey`, and the `onChanged`
	 * event) **plus** the custom Qevo helpers (`.api` facade, debug). Lazily
	 * instantiated and shares this runtime instance's debug setting.
	 *
	 * @example Native usage
	 * ```typescript
	 * const session = await qevo.runtime.cookies.get({
	 *   url: 'https://example.com',
	 *   name: 'session_id'
	 * });
	 *
	 * await qevo.runtime.cookies.set({
	 *   url: 'https://example.com',
	 *   name: 'preference',
	 *   value: 'dark_mode'
	 * });
	 *
	 * qevo.runtime.cookies.onChanged.addListener((changeInfo) => {
	 *   console.log(changeInfo.cookie.name, changeInfo.removed);
	 * });
	 * ```
	 *
	 * @example Custom Qevo facade
	 * ```typescript
	 * const nativeShaped = qevo.runtime.cookies.api;
	 * ```
	 */
	get cookies(): QevoCookies;
	/**
	 * Native runtime message event passthrough.
	 *
	 * Exposes the underlying browser `runtime.onMessage` event object directly so
	 * consumers get the same `addListener`, `removeListener`, `hasListener`, and
	 * `hasListeners` behavior as the native extension API.
	 *
	 * @example
	 * ```typescript
	 * qevo.runtime.onMessage.addListener((message, sender, sendResponse) => {
	 *   sendResponse({ ok: true });
	 *   return true;
	 * });
	 * ```
	 */
	get onMessage(): RuntimeOnMessageEvent;
	/**
	 * Fired when a message is sent from another extension (by
	 * {@link QevoRuntime.sendMessage}). Cannot be used in a content script.
	 *
	 * Returns an inert (no-op) event object if the browser does not support it.
	 *
	 * @example
	 * ```typescript
	 * qevo.runtime.onMessageExternal.addListener((message, sender, sendResponse) => {
	 *   if (sender.id === TRUSTED_EXTENSION_ID) sendResponse({ ok: true });
	 *   return true;
	 * });
	 * ```
	 */
	get onMessageExternal(): RuntimeOnMessageEvent;
	/**
	 * Fired when a connection is made from another extension (by
	 * {@link QevoRuntime.connect}), or from an externally connectable web site.
	 */
	get onConnectExternal(): RuntimeEvent<ConnectListener>;
	/**
	 ...
	 * @since Chrome 76
	 */
	get onConnectNative(): RuntimeEvent<ConnectListener>;
	/**
	 * Fired when a connection is made from a user script from this extension.
	 *
	 * @since chrome 115 MV3
	 */
	get onUserScriptConnect(): RuntimeEvent<ConnectListener>;
	/**
	 ...
	 * @since chrome 115, MV3
	 */
	get onUserScriptMessage(): RuntimeOnMessageEvent;
	/**
	 ...
	 * enforced after a 24-hour grace period has passed. Currently, this event is
	 * only fired for Chrome OS kiosk apps.
	 */
	get onRestartRequired(): RuntimeEvent<RestartRequiredListener>;
	/**
	 * Sent after onSuspend to indicate that the app won't be unloaded after all.
	 */
	get onSuspendCanceled(): RuntimeEvent<SuspendListener>;
	/**
	 * Fired when a Chrome update is available, but isn't installed immediately
	 * because a browser restart is required.
	 *
	 * @deprecated Please use {@link QevoRuntime.onRestartRequired}.
	 */
	get onBrowserUpdateAvailable(): RuntimeEvent<() => void>;
	/**
	 ...
	 * Observe this event to be proactively notified of runtime performance
	 * problems with the extension. (Firefox only.)
	 */
	get onPerformanceWarning(): RuntimeEvent<PerformanceWarningListener>;
	/**
	 * Returns the current extension's version.
	 *
	 ...
	 *
	 * Firefox only; resolves to `undefined` on Chromium browsers, which expose
	 * no equivalent.
	 *
	 * @returns The browser name, vendor, version, and build ID, or `undefined`.
	 */
	getBrowserInfo(): Promise<BrowserInfo | undefined>;
	/**
	 * Returns the frame ID of any window global or frame element when called in
	 * any context.
	 *
	 * Firefox only; returns `-1` on browsers that do not support it.
	 *
	 * @param target - A `Window` or frame element to retrieve the frame ID for.
	 * @returns The frame ID, or `-1` if it cannot be determined.
	 */
	...
	private ensureListenersInitialized;
	/**
	 * Resolve a native runtime event into a fully typed {@link RuntimeEvent}.
	 *
	 * Falls back to an inert, no-op event object when the running browser does
	 * not implement the event (for example, ChromeOS-only or Chrome-only events
	 * under Firefox), so consumer `addListener` / `removeListener` calls stay
	 * safe everywhere.
	 *
	 * @internal
	 */
